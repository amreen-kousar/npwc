...
const Diet = ({ navigation }) => {
  ...
  const getValues = async () => {
    ...
    let start_date = await AsyncStorage.getItem('dietstartDate')
    let end_date = await AsyncStorage.getItem('dietendDate')
          ...
          if (response?.data?.data) {
            ...
            axios.get(`https://novapwc.com/api/getOneDietPlan?userid=${userIdAsync}`)
              .then(function (response) {
                ...
                response.data.data = response?.data?.data.filter(e => e.type == 'food')
  ...
  const setDietId = (item) => {
    ...
    navigation.navigate('ItemsOfCategory',
      { cat: id, diet_id: item?.diet_id, category: item.category, type: "food", servingsConsumed: item.servings_consumed, apiCall: apiCall, recommendedServings: item.recommended_servings }
    )
  }
              ...
              (dietData?.Type) ? <View>
                <View style={[styles.cardIn, styles.mainCard]}>
                  <View style={styles.rowDirection}>
                    ...
                    <View style={[styles.card, styles.mainInCard]}>
                      <Text style={{ color: "black" }}>Today's Intake{"\n"}<Text style={styles.heading}>
                        {dietData?.RecommendedServings} </Text>servings</Text>
                    </View>
                  </View>
                </View>
                ...
                <View style={[styles.card, styles.blackCard, styles.rowDirection, { justifyContent: "space-around" }]}>
                  <View style={{ flexDirection: "row", alignItems: "center" }}>
                    {convertNumber(dietData.servingsLeft, "servings")}
                      ...
                      <View style={[styles.card, styles.categoryCard, styles.shadowProp, { marginStart: 5, marginEnd: 5 }]}>
                        ...
                        <View style={{
                          ...
                          <Text style={{ lineHeight: 20, fontSize: 11, flex: 1, color: "black" }}>
                            <Text style={{ fontSize: 15 }}>{item.servings_consumed}</Text> servings
                            consumed
