...
const ItemsOfCategory = ({ route, navigation }) => {
    ...
    const apiCall = async () => {
        ...
        axios.get(`https://novapwc.com/api/getItemsOfCategory?category_id=${route?.params.cat}&type=exercise`)
            .then(function (response) {
                console.log(response?.data?.data)
                if (response?.data?.data) {
                    items = response?.data?.data
                    setItems(items)
                    // console.log(`https://novapwc.com/api/itemIntakeStatus?userid=${userIdAsync}&type=exercise&category=${route?.params.category}`, "utllllll")
                    axios.get(`https://novapwc.com/api/itemIntakeStatus?userid=${userIdAsync}&type=exercise&category=${route?.params.category}`)
                        .then(function (response) {
